...
class PostCatalogueController extends Controller
{
    ...
    public function index()
    {
        ...
        $this->authorize('modules', 'posts.catalogues.index');
    ...
    function create()
    {
        $this->authorize('modules', 'posts.catalogues.create');
    ...
    public function store(StorePostCatalogueRequest $request)
    {
        if ($this->postCatalogueService->create()) {
            return redirect()->route('posts.catalogues.index')->with('toast_success', 'Tạo nhóm bài viết mới thành công.');
        }
        return redirect()->route('posts.catalogues.create')->with('toast_error', 'Có lỗi vui lòng thử lại.');
    }

    public function edit($id)
    {
        $this->authorize('modules', 'posts.catalogues.edit');
    ...
    public function update(UpdatePostCatalogueRequest $request, $id)
    {
        ...
        if (empty($idPostCatalogue)) {
            return redirect()->route('posts.catalogues.index')->with('toast_error', 'Có lỗi vui lòng thử lại.');
        }

        if ($this->postCatalogueService->update($idPostCatalogue)) {
            // Xoá giá trị sesson
            session()->forget('_id');
            return redirect()->route('posts.catalogues.index')->with('toast_success', 'Cập nhập nhóm bài viết thành công.');
        }
        // Xoá giá trị sesson
        session()->forget('_id');
        return redirect()->route('posts.catalogues.edit', $id)->with('toast_error', 'Có lỗi vui lòng thử lại.');
    }
    ...
    {
        $this->authorize('modules', 'posts.catalogues.destroy');

        if ($request->_id == null) {
            return redirect()->route('posts.catalogues.index')->with('toast_error', 'Có lỗi vui lòng thử lại');
        }
        if ($this->postCatalogueService->destroy($request->_id)) {
            return redirect()->route('posts.catalogues.index')->with('toast_success', 'Xoá nhóm bài viết thành công.');
        }
        return redirect()->route('posts.catalogues.index')->with('toast_error', 'Có lỗi vui lòng thử lại.');
    }
}
