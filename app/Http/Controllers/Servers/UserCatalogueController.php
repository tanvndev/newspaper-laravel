...
class UserCatalogueController extends Controller
{
    ...
    {
        $this->authorize('modules', 'users.catalogues.create');
    ...
    public function store(StoreUserCatalogueRequest $request)
    {
        ...
        if ($this->userCatalogueService->create()) {
            return redirect()->route('users.catalogues.index')->with('toast_success', $successMessage);
        }
        return redirect()->route('users.catalogues.create')->with('toast_error', $errorMessage);
    }

    public function edit($id)
    {
        $this->authorize('modules', 'users.catalogues.edit');
    ...
    public function update(UpdateUserCatalogueRequest $request, $id)
    {
        ...
        $idUser = session('_id');
        if (empty($idUser)) {
            return redirect()->route('users.catalogues.index')->with('toast_error', $errorMessage);
        }
    ...
    public function destroy(Request $request)
    {
        ...
        $successMessage = $this->getToastMessage('userCatalogue', 'success', 'delete');
        $errorMessage = $this->getToastMessage('userCatalogue', 'error', 'delete');
    ...
    public function updatePermission(Request $request)
    {
        ...
        if ($this->userCatalogueService->setPermission()) {
            return redirect()->route('users.catalogues.permission')->with('toast_success', $successMessage);
        }
        return redirect()->route('users.catalogues.permission')->with('toast_error', $errorMessage);
    }
}
